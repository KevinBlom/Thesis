...
The presented research question prompts thorough investigation of the connection between the pressure that a tap exerts on a touch screen and the emotional state or mood of the user that performs the taps.
% \begin{itemize}
...
\label{sub:findings}
A multiple regression was run to predict either valence or arousal from maximum tap pressure and duration, or average tap pressure and duration. Table \ref{tab:findings} shows the model summary. In each seperate case, none of the independent variables significantly predicted valence nor arousal, $p > .05$. (See Appendix Regression Coefficients)
